...
print(count_lines(local_folder + "output.txt"))
print(count_lines(local_folder + "extracted-strings.txt"))

if count_lines(local_folder + "output.txt") == count_lines(local_folder + "extracted-strings.txt"): 
    print("Seems OK")
    subprocess.run([a24_ebm, "--replace-strings", ebm_folder + "/" + file_to_fix, local_folder + "output.txt"], cwd=local_folder)
